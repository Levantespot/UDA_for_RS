...
class DACS(UDADecorator):
    ...
    def mix_source_img(self, img_batch, gt_semantic_seg_batch, type):
        ...
        all_classes = torch.arange(self.get_model().num_classes)

        if type == 'MinorClassMix':
            for idx, (img, gt) in enumerate(zip(img_batch, gt_semantic_seg_batch)):
                img_aug, gt_aug = img.clone(), gt.clone()
                for cls in all_classes:
                    if (gt == cls).sum() / (img_size - (gt==ignore_index).sum()) < self.mix_class_threshold:
                        to_pick_idxs = [_idx for _idx in np.arange(batch_size) if _idx != idx] # pick index of another image
                        if len(to_pick_idxs) == 0: continue # pass if no image contains that class
                        mixed_idx = np.random.choice(to_pick_idxs)

                        mask = (gt_semantic_seg_batch[mixed_idx] == cls).long()
                        img_aug = (1-mask) * img_aug + mask * img_batch[mixed_idx]
                        gt_aug = (1-mask) * gt_aug + mask * gt_semantic_seg_batch[mixed_idx]
                mixed_img_batch[idx] = img_aug
                mixed_gt_batch[idx] = gt_aug
        elif type == 'MajorClassMix':
            for idx, (img, gt) in enumerate(zip(img_batch, gt_semantic_seg_batch)):
                img_aug, gt_aug = img.clone(), gt.clone()
                for cls in all_classes:
                    if (gt == cls).sum() / (img_size - (gt==ignore_index).sum()) >= self.mix_class_threshold:
                        ...
                        img_aug = (1-mask) * img_aug + mask * img_batch[mixed_idx]
                        gt_aug = (1-mask) * gt_aug + mask * gt_semantic_seg_batch[mixed_idx]
                        # debug_items.append((img_aug, gt_aug))
                mixed_img_batch[idx] = img_aug
                mixed_gt_batch[idx] = gt_aug
        elif type == 'CutMix':
            ...
            for idx, (img, gt) in enumerate(zip(img_batch, gt_semantic_seg_batch)):
                cls_under_thre = [cls for cls in all_classes if cls not in gt_class_pros[idx] or \
                    gt_class_pros[idx][cls] < self.mix_class_threshold]
                for cls in cls_under_thre:
                    idx_list = [_idx for _idx in class_with_idx[cls] if _idx != idx]
                    if len(idx_list) == 0: continue # pass if it's only one
                    random.shuffle(idx_list)
                    mixed_idx = idx_list[0]
                    mask = (gt == cls).long()
                    img = mask * img + (1-mask) * img_batch[mixed_idx]
                    gt = mask * gt + (1-mask) * gt_semantic_seg_batch[mixed_idx]
                mixed_img_batch[idx] = img
                mixed_gt_batch[idx] = gt
        elif type == 'MinorClassMix2':
            minor_classes = torch.arange(2,6)
            ...
            for idx, (img, gt) in enumerate(zip(img_batch, gt_semantic_seg_batch)):
                ...
                mixed_img_batch[idx] = img_aug
                mixed_gt_batch[idx] = gt_aug
        elif type == 'WarmMinorClassMix':
            if self.local_iter < 1500:  # pass
                return img_batch, gt_semantic_seg_batch
            for idx, (img, gt) in enumerate(zip(img_batch, gt_semantic_seg_batch)):
                img_aug, gt_aug = img.clone(), gt.clone()
                for cls in all_classes:
                    if (gt == cls).sum() / (img_size - (gt==ignore_index).sum()) < self.mix_class_threshold:
